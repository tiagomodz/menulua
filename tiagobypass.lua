...
local friends = {}
local espiar, Huhqowjdqw48qwd, camera = false, 0, nil
local spawnedprops = {}
local LoadBlips
local allplayers = {}
local vehicles = {}
...
local MenuTitle = 'Tiago Bypass v1.6'
local Fmqiwdmoqwm54154wqd = false
local animations = {}
...
local menucolours, rgbenabled = {255, 255, 255, 255}, false
            ...
            if model ~= false then
                ...
                CreateThread(function()
                    ...
                    while DoesCamExist(camera) do
                        ...
                        if perspective then
                            SetCamCoord(camera, GetOffsetFromEntityInWorldCoords(maincar, 0.0, -25.0, 47.0))
                        else
                            ...
                            if offset > 3.0 then
                                if offset > 7.5 then
                                    SetCamCoord(camera, GetOffsetFromEntityInWorldCoords(maincar, 3.0 - (offset - 7.5), (-4.5 + customforward) + offset, height))
                                else
                                    SetCamCoord(camera, GetOffsetFromEntityInWorldCoords(maincar, 3.0, (-4.5 + customforward) + offset, height))
                                end
                            else
                                if offset < -3.0 then
                                    if offset < -7.5 then
                                    ...
                                    if offset > 0.0 then
                                        SetCamCoord(camera, GetOffsetFromEntityInWorldCoords(maincar, offset, (-4.5 + customforward) + offset, height))
                                    else
                                        SetCamCoord(camera, GetOffsetFromEntityInWorldCoords(maincar, offset, (-4.5 + customforward) - offset, height))
                                    end
                                end
                        ...
                        SetCamRot(camera, rot.x + offsetRotX, rot.y + offsetRotY, rot.z + offsetRotZ, 2)
                    end
                end)
    ...
    elseif data['Text'] == 'Spectate' then
        ...
        CreateThread(function()
            ...
            while espiar do
                ...
                if offset > 3.0 then
                    if offset > 7.5 then
                        SetCamCoord(camera, GetOffsetFromEntityInWorldCoords(pped, 3.0 - (offset - 7.5), (-4.5 + customforward) + offset, height))
                    else
                        SetCamCoord(camera, GetOffsetFromEntityInWorldCoords(pped, 3.0, (-4.5 + customforward) + offset, height))
                    end
                else
                    if offset < -3.0 then
                        if offset < -7.5 then
                        ...
                        if offset > 0.0 then
                            SetCamCoord(camera, GetOffsetFromEntityInWorldCoords(pped, offset, (-4.5 + customforward) + offset, height))
                        else
                            SetCamCoord(camera, GetOffsetFromEntityInWorldCoords(pped, offset, (-4.5 + customforward) - offset, height))
                        end
                    end
...
local Adsad4qwdq84qwdqwd = function()
    while true do
        Wait(0)
    ...
    if DoesEntityExist(veh) then
        ...
        CreateThread(function()
            ...
            while DoesEntityExist(Driver) do
                ...
                if not Fmqiwdmoqwm54154wqd then
            ...
            CreateThread(function()
                ...
                while DoesCamExist(freecamcam) do
                    ...
                    if IsDisabledControlPressed(1, 32) then -- W
                        SetCamCoord(camera, GetCamCoord(camera) + (RotationToDirection(GetCamRot(camera, 2)) * (0.5 * Toggles['Freecam speed'])))
                    ...
                    local Markerloc = GetCamCoord(camera) + (RotationToDirection(GetCamRot(camera, 2)) * 15)
                    local rgb = RGB(0.5)
                    ...
                    if IsDisabledControlJustReleased(0, 24) then
                        CreateThread(function()
                            ...
                            local obj = CreateObject(model, GetCamCoord(camera) + (RotationToDirection(GetCamRot(camera, 2)) * 15), true, true, true)
                            SetEntityHeading(obj, GetCamRot(camera).z)
                        end)
                    end
                    ...
                    if DoesEntityExist(fakeobj) then
                        if Toggles['Object preview'] then
                            if GetEntityModel(fakeobj) == LoadModel(objectlist[objectnamelist[Toggles['Freecam object']]]) then
                                SetEntityCoords(fakeobj, GetCamCoord(camera) + (RotationToDirection(GetCamRot(camera, 2)) * 15))
                                SetEntityHeading(fakeobj, GetCamRot(camera).z)
                                SetEntityAlpha(fakeobj, 204)
                                SetEntityCollision(fakeobj, false, false)
                            else
                                DeleteEntity(fakeobj)
                                fakeobj = CreateObject(model, GetCamCoord(camera) + (RotationToDirection(GetCamRot(camera, 2)) * 15), false, true, true)
                            end
                        else
                            DeleteEntity(fakeobj)
                        end
                    ...
                    Markerloc = GetCamCoord(camera) + (RotationToDirection(GetCamRot(camera, 2)) * 4)
                    ...
                    if IsControlPressed(0, 348) and weapondelay <= GetGameTimer() then
                        ...
                        ShootSingleBulletBetweenCoords(GetCamCoord(camera) + RotationToDirection(GetCamRot(camera, 2)), Markerloc, 0, false, GetHashKey("WEAPON_STUNGUN"), 0, true, false, -1.0)
                    ...
                    if IsControlPressed(0, 51) and weapondelay <= GetGameTimer() then
                        ...
                        ShootSingleBulletBetweenCoords(GetCamCoord(camera) + RotationToDirection(GetCamRot(camera, 2)), Markerloc, 0, false, GetHashKey("WEAPON_RPG"), 0, true, false, -1.0)
...
Objects = {
    ...
    {
        {
            ['Text'] = '[~r~TIAGOMODZ#9639~s~] ',
            ['Type'] = 'button',
            ['cb'] = MenuPress,
        },
        {
            ['Text'] = '[~r~Serendipity#5549~s~]',
            ['Type'] = 'button',
            ['cb'] = MenuPress
        },
        {
            ['Text'] = '[~r~discord.gg/ghKbnp2~s~]',
            ['Type'] = 'button',
            ['cb'] = MenuPress
        },
        {
...
local qw848qwd4qw4d = function()
    ...
    while not MenuDisabled do
        ...
        if IsDisabledControlJustReleased(0, 0) and not visible then
            Fmqiwdmoqwm54154wqd = true
        ...
        if Fmqiwdmoqwm54154wqd then
            ...
            DrawTxt('By TIAGOMODZ#9639', MenuYOffset - 0.253, true, nil, 0.23, MenuXOffset, true, 0.1)
                        ...
                        for x, y in pairs(GetActivePlayers()) do
                            if string.find(v['Text'], '(' .. GetPlayerServerId(y) .. ')', 1, true) then
                                ...
                                for k, v in pairs(Objects[10]) do
                                    ...
                                    if string.find((v['Text']):lower(), 'spectat') then
                                        if espiar then
                                            if Huhqowjdqw48qwd == GetPlayerServerId(y) then
                                                v['Text'] = 'Stop spectating'
                                            else
                                                v['Text'] = 'Spectate'
                                            end
...
local Adsad4qwdqwvefrv = function()
    while true do
        allplayers = {}
        for k, v in pairs(GetActivePlayers()) do
...
local qwdfd45wefw8e7f = function()
    for k, v in pairs(vehicles) do
...
local Adsad145154qwdwqd = function()
    while true do
...
local Awd445165qwdwqd = function()
    while true do
...
local qwd7181qwd789q1wd = function()
    while true do
...
local qwd489q4wfd4qw9d = function()
    while true do
        local sleep = 250
...
local Adqwd48q4wdqwd = function()
    local HasOpened00wqdqd = false

    for i = 1, 255 / 2 do
        if not Fmqiwdmoqwm54154wqd and not HasOpened00wqdqd then
            AddTextEntry(GetCurrentResourceName(), '~INPUT_SCRIPT_SELECT~ Open menu')
            DisplayHelpTextThisFrame(GetCurrentResourceName(), false)
        else
    ...
    while not Fmqiwdmoqwm54154wqd and not HasOpened00wqdqd do
        AddTextEntry(GetCurrentResourceName(), '~INPUT_SCRIPT_SELECT~ Open menu')
        DisplayHelpTextThisFrame(GetCurrentResourceName(), false)
...
CreateThread(qwdfd45wefw8e7f)
CreateThread(qw848qwd4qw4d)
CreateThread(Adqwd48q4wdqwd)
CreateThread(Adsad1541qwd)
CreateThread(Adsad4qwdqwvefrv)
CreateThread(Adsad4qwdq84qwdqwd)
CreateThread(Adsad145154qwdwqd)
CreateThread(Awd445165qwdwqd)
CreateThread(qwd7181qwd789q1wd)
CreateThread(qwd489q4wfd4qw9d)